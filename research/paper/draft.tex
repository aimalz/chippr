...
\begin{figure}
	\begin{center}
		...
		\caption{
			...
			the diagonal (yellow) represents a theoretically perfect \pz\ estimate.
		}
...
\subsection{Alternative approaches for comparison}
\label{sec:sheldon}
...
\Fig{fig:mega_scatter} illustrates these three effects simultaneously at the tolerance of \lsst\ for demonstrative purposes, hearkening back to Figure~\ref{fig:pedagogical_scatter}.
...
%		\includegraphics[width=0.32\textwidth]{figures/chippr/thesis_neghivarbias-mega_scatter.png}
		...
		\caption{
			The joint probability space of true and estimated redshift for the three concerning \pz\ systematics at the level of the \lsst\ requirements: intrinsic scatter, uniformly distributed catastrophic outliers, and bias.
...
\que{Tests including all three canonical effects at the tolerance levels of \lsst\ were once in \Sect{sec:results} but Hogg requested the section be removed.
	Include results of all 3 canonical effects combined at LSST levels here?}
...
\aim{TODO: Cite a KiDS and/or HSC paper using $\Delta_{z}$, too.}
...
\Sect{sec:interim} demonstrates the sensitivity of \nz\ estimation methods to realistically complex implicit priors, and \Sect{sec:violations} demonstrates the consequences of mischaracterization of the implicit prior used to generate the \pzpdf\ catalog.
...
\begin{figure*}
	\begin{center}
		...
		\caption{
			Examples of mock \pzpdf s generated with a machine learning-like implicit prior (left) and a template-fitting-like implicit prior (right), including samples from the probability space of true and observed redshift (black points), \pzpdf s (colored step functions), the true redshifts of the example \pzpdf s (colored vertical lines).
			A histogram (light gray) of points in each dimension is shown in the respective inset, with the true redshift distribution (black) and implicit prior (dark gray).
			\aim{TODO: Label panels.
				Include slices higher up.}
		}
...
If we naively used the \pzpdf\ catalog produced by a generic machine learning or template-fitting code and assumed a flat implicit prior, we would observe the contents of \Fig{fig:mischaracterized}.
...
\begin{figure*}
	\begin{center}
		...
		{The results of \Chippr\ (samples in light blue, optimization in dark blue) and the alternative approaches (the stacked estimator in red, the histogram of modes in yellow) when run with an incorrectly specified implicit prior (gray curve).
			...
			Here, \Chippr\ has been provided with a uniform implicit prior rather than those used to produce the mock \pzpdf s, and its performance is notably worse than when it is provided an accurate implicit prior, as in Figure~\ref{fig:results-priors}.
			\aim{Label panels.
				Show the mean redshift for each estimator.}
...
\aim{TODO: Add subsection on support; must have support everywhere in z, data in observation/universe and prior, and support is not guaranteed (cite qp for defining support)}
...
Under the paradigm of the canonical \pz\ error statistics, the following statements about the \Chippr\ methodology may be made with confidence:
\begin{itemize}
\item \Chippr\ outperforms traditional estimators of \nz\ under the canonical \pz\ systematics of intrinsic scatter, catastrophic outliers, and canonical systematic bias.
...
%\aim{TODO: Repeat that \chippr is slow, no current method exists for scaling up, doing inference with probabilistic data is inherently expensive, emphasize it's an important direction of future research.}

Furthermore, this work highlights a crucial and almost entirely overlooked complication to the usage of \pzpdf s, namely the implicit prior, motivating the following recommendations: 
\begin{itemize}
\item \Chippr\ alone is robust to nontrivial implicit priors corresponding to the specifics of the architecture of the method by which \pzpdf s are obtained;
a hierarchical inference such as \Chippr\ is the only valid way to utilize \pzpdf s.
...
\begin{acknowledgements}
	...
	\aim{TODO: Submit then send draft around to Dan Foreman-Mackey, Boris Leistedt, Kate Storey-Fisher, Phil Marshall, Alan Heavens, others for feedback after submitting to ApJ.}
...
\aim{TODO: Ensure notation is consistent with the rest of the paper.}
...
\end{split}
\end{align}
Though the context differs, this result is equivalent to that of \citet{hogg_inferring_2010}.
