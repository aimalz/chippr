...
\begin{figure}
	\begin{center}
		...
		\caption{
			\aim{TODO: Change y axis label to ``nonlinear projection of photometric data'' and y axis to ``true redshift.''}
			...
			\aim{TODO: Specify relationship between my figure and Jain figure; points are spec-z as true z and photo-z as nonlinear projection of photometric data.}
		}
...
When we constrain the redshift of a galaxy using its observed photometric data $\data_{j}$, we are effectively estimating a posterior $\pr{z \gvn \data_{j}}$, the probability of an unknown quantity conditioned on the quantity we have in hand, i.e the data.
...
\aim{TODO: Call the joint distribution an ``empirical frequency distribution'' and clarify that it exists in nature but is not known.}
\aim{TODO: Show this conditional on the true redshift distribution $\phi^{\dagger}$.}
\que{Propagate new $\phi^{\dagger}$ notation through appendix?}

However, to estimate a posterior $\pr{z \gvn \data_{j}}$, the investigator must have a model $\phi$ for the general relationship between redshifts and photometry, whether empirical, as is the case for machine learning \pzpdf\ methods, or analytic, as is the case for template-based \pzpdf\ methods.
...
\begin{figure*}
	\begin{center}
		...
		\caption{
			\aim{TODO: Replace with figure of all effects at the same time.
				Include slice further up so we can see outliers.}
...
\aim{TODO: formally define this type of \pz\ bias and call it ``canonical'' or something for future distinction from generic biases.}
...
As expected based on consistency with the forward model, \Chippr\ is completely resistant to linear bias of the form of $\Delta_{z}$, and the alternative estimators are only weakly affected, with information loss two and four times greater than that of the \Chippr\ \mmle\ for the histogram of modes and stacked estimator respectively.
\aim{Clarify that provided p(z) is consistent with forward model, CHIPPR has no bias of the canonical form.}
...
\aim{TODO: Move conclusions opening and list of points here.}
...
\aim{TODO: Break up conclusion into subsections of \Sect{sec:results}.}
...
\begin{acknowledgements}
	...
	\aim{TODO: Submit then send draft around to Foreman-Mackey, Leistedt, Kate Storey-Fisher, Phil Marshall, others for feedback after submitting to journal, post to arxiv at end of AAS.}
...
\aim{TODO: State that this is equivalent to the result of Adam Meyers, Bovy, Hogg.}
