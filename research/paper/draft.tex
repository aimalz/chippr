...
This plot looks very much like the traditional $z_{\mathrm{spec}}$ versus $z_{\mathrm{phot}}$ plots of \pz\ point estimates and was indeed adapted from one such plot in \citet{jain_whole_2015}, because \pz\ point estimates effectively are a special nonlinear projection of the data.
...
The prevailing way to estimate \nz\ for a sample of galaxies $j$ is to ``stack'' \pzpdf s according to Equation~\ref{eqn:stack}.
...
However, it must be noted here that Equation~\ref{eqn:stack} 
...
\end{equation}
the heart of \Chippr.
The derivation is provided in Appendix~{app:math}.
...
To test the impact of these uncertainties, we simulate mock data with all three effects with which \lsst\ is concerned at the levels of Table~\ref{tab:lsstsrd} 
