...
The most prevalent application of \pzpdf s is their use in estimating the \textit{redshift distribution function \Nz}, or, interchangably, its normalized cousin the \textit{redshift density function \nz}, a quantity essential to the calculation of the power spectra of weak gravitational lensing and large-scale structure that are used to constrain the parameters of cosmological models \citep{mandelbaum_precision_2008, hildebrandt_cfhtlens:_2012, sheldon_photometric_2012, benjamin_cfhtlens_2013, kelly_weighing_2014, bonnett_using_2015,  masters_mapping_2015, viironen_high_2015, asorey_galaxy_2016, bonnett_redshift_2016, leistedt_hierarchical_2016}, as well as to validate survey selection functions used in generation of realistic, multi-purpose mock catalogs \citep{norberg_2df_2002}.  

\aim{TODO:
	Motivate how well we need to know \Nz\ and why it matters.
	Provide equations/citations for the use of \Nz\ in cosmology.
...
However, any procedure that reduces \pzpdf s to point estimates loses information about the uncertainty on redshift.
To circumvent this loss, it has grown to be more common to calculate the conceptually simple but mathematically inconsistent \citep{hogg_data_2012} \textit{stacked estimator} $\hat{n}(z)$ of the redshift density function \citep{lima_estimating_2008}
\begin{align}
\label{eqn:stack}
...
\begin{figure}
	...
	\caption{
		\aim{TODO: Add watermark of "mock data" in UL corner, same for "results of inference" on other kind of plot.}
...
\aim{Is the distinction between binned samples determined by some observational property and probabilistic redshift distributions sufficiently clear?}
...
\aim{Does this discussion adequately quantify how good \Nz\ has to be and how wrong we'll be if we estimate it wrong?
...
\begin{figure}
	\begin{center}
		...
		\caption{
			\aim{TODO: same size axis labels on left and right, watermark with test conditions and ``mock data'' vs ``results of inference''}
